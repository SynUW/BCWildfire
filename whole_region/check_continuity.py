...
def detect_folder_date_format(folder_path):
    """检测文件夹中日期格式，返回strftime格式串："%Y%m%d" 或 "%Y_%m_%d"。
    若两种均未匹配，默认返回"%Y_%m_%d"。
    """
    count_compact = 0   # %Y%m%d
    count_underscored = 0  # %Y_%m_%d
    ...
    if count_compact == 0 and count_underscored == 0:
        return "%Y_%m_%d"
    return "%Y%m%d" if count_compact >= count_underscored else "%Y_%m_%d"
...
def fill_missing_with_zeros(folder_path, start_date_str, end_date_str):
    ...
    total_created = 0
    # 检测输出命名的日期格式，与文件夹内一致
    output_date_fmt = detect_folder_date_format(folder_path)
    ...
    for factor, info in missing_info.items():
        ...
        for missing_date in tqdm(info['missing_dates'], desc=f"处理 {factor}"):
            # 使用与文件夹一致的日期格式
            output_file = f"{factor}_{missing_date.strftime(output_date_fmt)}.tif"
...
if __name__ == "__main__":
    input_folder = "/mnt/raid/zhengsen/wildfire_dataset/self_built_materials/MODIS_LST_29_31_32_QC/MOD21A1DN_multibands_withoutFiltering_merged"
